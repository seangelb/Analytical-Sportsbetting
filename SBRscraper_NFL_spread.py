...
def parse_and_write_data(soup, date, not_ML=True):
    ...
    if number_of_games != 0:
        for i in range(0, number_of_games):
            ...
            tenbet_A = try_except_book_line('1157', i, 0)
            ...
            tenbet_H = try_except_book_line('1157', i, 1)
            justbet_H = try_except_book_line('1275',i,1)
            # print(i, pinnacle_H == fivedimes_H == heritage_H == betonline_H == tenbet_H == justbet_H == "")
            if pinnacle_H == fivedimes_H == heritage_H == betonline_H == tenbet_H == justbet_H == "":
                skip = skip + 1
                # print("skip")
                pass
            else:
                A.append(str(date) + '_' + A_names[i - skip] + '_' + H_names[i - skip])
                A.append(date)
                A.append(time)
                A.append('away')
                A.append(A_names[i - skip])
                A.append(H_names[i - skip])
                #print(i, A, pinnacle_A)
                if not_ML:
                    ...
                    tenbet_A = replace_unicode(tenbet_A)
                    tenbet_A_line = tenbet_A[:tenbet_A.find(' ')]
                    tenbet_A_odds = tenbet_A[tenbet_A.find(' ') + 1:]
                    A.append(tenbet_A_line)
                    A.append(tenbet_A_odds)
                    justbet_A = replace_unicode(justbet_A)
                    justbet_A_line = justbet_A[:justbet_A.find(' ')]
                    justbet_A_odds = justbet_A[justbet_A.find(' ') + 1:]
                    A.append(justbet_A_line)
                    A.append(justbet_A_odds)
                else:
                    ...
                    A.append(replace_unicode(tenbet_A))
                    A.append(replace_unicode(justbet_A))
                H.append(str(date) + '_' + A_names[i - skip] + '_' + H_names[i - skip])
                H.append(date)
                H.append(time)
                H.append('home')
                H.append(H_names[i - skip])
                H.append(A_names[i - skip])
                ...
                if not_ML:
                    ...
                    tenbet_H = replace_unicode(tenbet_H)
                    tenbet_H_line = tenbet_H[:tenbet_H.find(' ')]
                    tenbet_H_odds = tenbet_H[tenbet_H.find(' ') + 1:]
                    H.append(tenbet_H_line)
                    H.append(tenbet_H_odds)
                    justbet_H = replace_unicode(justbet_H)
                    justbet_H_line = justbet_H[:justbet_H.find(' ')]
                    justbet_H_odds = justbet_H[justbet_H.find(' ') + 1:]
                    H.append(justbet_H_line)
                    H.append(justbet_H_odds)
                else:
                    ...
                    H.append(replace_unicode(tenbet_H))
                    H.append(replace_unicode(justbet_H))
...
def select_and_rename(df, text):
    ...
    if text[-2:] == 'ml':
        ...
        df = df[['key', 'time', 'team', 'opp_team',
                 ...
                 'bovada_line', 'bovada_odds',
                 'betonline_line', 'betonline_odds',
                 'tenbet_line', 'tenbet_odds',
                 'justbet_line', 'justbet_odds']]
        ...
        df.columns = ['key', text + '_time', 'team', 'opp_team',
                      ...
                      text + '_TEN_line', text + '_TEN_odds',
                      text + '_JUS_line', text + '_JUS_odds']
    return df
...
def blank_out_df(df, text):
    ...
    if text[-2:] == 'ml':
        ...
        ## Change column names to make them unique
        df.columns = ['key', text + '_time', 'team', 'opp_team',
                      text + '_PIN', text + '_FD', text + '_HER', text + '_BVD', text + '_BOL', text + '_BET',
                      text + '_BOD']
